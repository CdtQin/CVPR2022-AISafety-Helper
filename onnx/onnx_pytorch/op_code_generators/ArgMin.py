...
class ArgMinOpCodeGenerator(OpCodeGenerator):

  def __init__(self,
               onnx_ver=onnx.defs.onnx_opset_version(),
               torch_ver=torch.__version__):
    super(ArgMinOpCodeGenerator, self).__init__(onnx_ver, torch_ver)
  ...
  def gen(self, node, value_infos, initializers):
    ...
    forward_str.append(
        f"{outputs_str[0]} = torch.argmin({', '.join(inputs_str)}, **{{{params_str}}})"
    )
