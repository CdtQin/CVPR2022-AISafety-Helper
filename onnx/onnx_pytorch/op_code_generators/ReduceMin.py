...
class ReduceMinOpCodeGenerator(ReduceOpCodeGenerator):

  def __init__(self,
               onnx_ver=onnx.defs.onnx_opset_version(),
               torch_ver=torch.__version__):
    super(ReduceMinOpCodeGenerator, self).__init__(onnx_ver, torch_ver)
  ...
  def gen(self, node, value_infos, initializers):
    ...
    for d in reversed(dim):
      forward_str.append(
          f"{outputs_str[0]}, _ = torch.min({curr_input}, {d}, **{{{params_str}}})"
      )
